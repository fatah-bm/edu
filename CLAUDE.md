...
- `sirkuit-sederhana/` (folder historis, tapi judul & isi halamannya sekarang "Pipa Air" — tidak di-rename, sama seperti alasan `tambahkali` tetap dipertahankan namanya) — simulasi **aliran air lewat pipa & gerbang**, pengenalan intuitif AND/OR/NOT **tanpa istilah formal**. Awalnya dibuat dengan tema saklar+lampu dan bentuk tetap (fixed shapes) serta "saklar terbalik" untuk NOT, tapi diganti total setelah masukan: tema lampu/saklar dirasa membingungkan, dan "saklar terbalik" terasa aneh secara konsep.
  - **Topologi acak generik** (bukan bentuk tetap) — `randomTopology(n, depth, invertBudget, nextId)` membangun pohon `{type:'switch'|'series'|'parallel', ...}` secara rekursif: tiap simpul internal punya 2 anak yang ukurannya (jumlah gerbang) dibagi acak, tipe (seri/paralel) dipilih acak, sehingga seri & paralel bisa bergabung berulang dalam kombinasi yang bervariasi. Kedua tujuan (isi ember / jangan isi ember) matematis selalu bisa dicapai untuk topologi apa pun (dibuktikan via induksi: series butuh semua anak "true"-achievable, parallel cukup satu) — makanya tidak perlu BFS solvability check seperti `labirin-kode` (untuk model graf penuh dengan fan-in/fan-out & multi-sumber yang lebih rumit, lihat item roadmap terkait — proof induktif ini tidak lagi berlaku di sana).
  - **Satu akar, banyak daun (multi-ember)** — soal 1-4 pakai `SINGLE_LEVELS[index]` (`{n, depth, invert}` tetap, kesulitan naik manual sampai `depth:4` di soal tersulit) = satu pohon, satu ember (`generateLevel` membungkusnya jadi `{shared: null, branches: [satu branch]}`); soal 5-8 pakai `MULTI_LEVELS[index-4]` = satu topologi gerbang **bersama** (opsional, `sharedN` gerbang) yang bercabang ke beberapa **branch** independen, masing-masing dengan topologi & `goal` sendiri, berakhir di ember bernomor sendiri. Ini BUKAN graf/DAG — tetap pohon (satu akar, banyak daun), jadi proof `assignForTarget` tetap berlaku utuh per-cabang.
  - **Struktur multi-ember diacak tiap sesi, bukan tetap** — tiap entri `MULTI_LEVELS` menyimpan rentang `[min,max]` (`sharedN`, `sharedDepth`, `branchCount`, `branchN`, `branchDepth`, `branchInvert`), di-`randInt()` ulang tiap kali `generateLevel()` dipanggil (tiap kali "Main Lagi") — jumlah cabang (2 atau 3), ada/tidaknya & besar gerbang bersama, dan kedalaman tiap cabang semua bervariasi antar sesi, bukan cuma bentuk pohon & `goal`-nya saja seperti sebelumnya. Soal tersulit (`MULTI_LEVELS[3]`) bisa mencapai `branchDepth` hingga 3 dan `sharedDepth` hingga 2, sehingga total "kedalaman" dari sumber ke ember bisa mencapai 4-5 level. Diverifikasi lewat simulasi Node 20.000 ronde × 8 soal (160.000 pengecekan per-cabang) tanpa satu pun pelanggaran jaminan solvable/initial-mismatch, dengan kedalaman pohon maksimum yang benar-benar tercapai = 4 (sesuai `depth` tertinggi di `SINGLE_LEVELS`).
  - `buildBranch(bcfg, nextId)` membangun satu `{topology, switches, hasInverted, goal, label}`; `branchReaches(branch)` mengevaluasi apakah air benar-benar sampai ke ember itu: `(shared ? evalNode(shared) : true) && evalNode(branch.topology)` — gerbang bersama yang tertutup otomatis memblokir SEMUA ember di bawahnya, mendemonstrasikan bahwa satu gerbang bisa memengaruhi banyak hasil sekaligus.
  - **Render generik via SVG**, bukan CSS flexbox — `measure(node)` menghitung lebar (unit seri) & tinggi (jumlah jalur paralel) tiap simpul secara rekursif. `renderNode(node, x, y, w, h, incoming)` menggambar rekursif ke koordinat SVG (garis `<line>` sebagai pipa, gerbang sebagai `<button>` HTML di dalam `<foreignObject>` supaya tetap bisa di-tap) dan mengembalikan `{svg, outgoing}` — **`incoming` dan `outgoing` di-thread eksplisit lewat seluruh rekursi** (series meneruskan `outgoing` anak sebelumnya sebagai `incoming` anak berikutnya; parallel menyiarkan `incoming` yang sama ke semua cabang lalu `outgoing = branchOutgoings.some(v=>v)`), supaya status "mengalir" tiap kabel mencerminkan **jangkauan air yang sebenarnya dari sumber**, bukan cuma status lokal gerbang itu sendiri. Ini adalah perbaikan bug: versi sebelumnya mewarnai tiap kabel berdasarkan `evalNode()` subtree lokal saja, sehingga air bisa tampak "mengalir" di sebuah gerbang padahal gerbang SEBELUMNYA (hulu) sedang tertutup. `renderCircuitBoard()` merender `shared` dulu (kalau ada, `incoming` awal selalu `true` dari sumber 🚰), lalu memakai `outgoing`-nya sebagai `incoming` untuk tiap `branches[i]`, menggambar percabangan visual (garis fork + `junctionDot`) hanya kalau `branches.length > 1`, dan memberi label angka (`.bucket-num`, "1"/"2"/"3") di atas tiap ember hanya saat multi-ember.
  - **Margin kiri-kanan simetris (`MARGIN_X`)** — posisi ember (`bucketX = x + forkPxW + MARGIN_X/2`, `svgW = x + forkPxW + MARGIN_X`) sengaja dicerminkan dari posisi ikon sumber 🚰 di sisi kiri (yang juga berjarak `MARGIN_X` dari tepi SVG). Sebelumnya sisi kanan pakai angka tetap `+26`/`+70` (hasil tuning untuk tampilan satu-ember lama) yang kebetulan hampir 2x `MARGIN_X` — begitu papan melebar untuk multi-ember, seluruh diagram tampak "nempel" ke kiri dengan sisa ruang kosong besar di kanan. Prinsipnya: berapa pun bentuk pohonnya, jarak dari tepi SVG ke elemen pertama/terakhir (ikon sumber/ember) harus selalu sama di kedua sisi.
  - **Gerbang normal** (🚪): tap untuk toggle BUKA/TUTUP, air lewat kalau BUKA. **Gerbang terbalik** (🔀, border putus-putus): kebalikannya — air lewat kalau TUTUP. Ini pengenalan NOT: dibingkai sebagai "gerbang yang terbalik cara kerjanya" (dijelaskan lewat hint sekali di atas papan) bukan sebagai "saklar rusak", supaya terasa seperti mekanik permainan yang disengaja, bukan sesuatu yang aneh.
  - Anak men-tap gerbang untuk toggle (`toggleSwitch()` cuma flip state lalu render ulang seluruh papan lewat `renderCircuitBoard()`, tanpa tombol "jalankan" terpisah), lalu tekan "✅ Cek" untuk submit ke `goal` tiap ember. Tidak ada penalti untuk jawaban salah (mendorong bereksperimen), mengikuti filosofi `labirin-kode`. `checkAnswer()` mengecek SEMUA `branches` sekaligus (`allCorrect`) — kalau ada yang salah pada level multi-ember, feedback menyebutkan nomor ember yang belum sesuai (mis. "🤔 Ember 2, 3 belum sesuai, coba lagi!"); level satu-ember tetap pakai pesan generik lama.
  - **Kondisi awal gerbang sengaja dibuat berlawanan dari tujuan** lewat `assignForTarget(node, target, out)` (bukan `Math.random()` polos per gerbang) — supaya soal tidak pernah kebetulan sudah benar saat dimuat. Untuk level satu-ember: `assignForTarget(branch.topology, !branch.goal, initState)`. Untuk level multi-ember: `shared` dipaksa `true` (terbuka, supaya tidak jadi penentu tunggal yang membingungkan di awal) lalu tiap `branch` dipaksa `!branch.goal` sendiri-sendiri — hasilnya semua ember dijamin salah sekaligus di awal. `assignForTarget()` membangun mundur dari hasil yang diinginkan secara rekursif (series/parallel tahu persis kombinasi anak mana yang "menentukan" hasil, sisanya diacak bebas) — dijamin matematis lewat proof yang sama seperti soal selalu solvable, diverifikasi lewat simulasi puluhan ribu kombinasi (termasuk kasus multi-ember) tanpa satu pun re-match.
