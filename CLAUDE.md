...
- `sirkuit-sederhana/` (folder historis, tapi judul & isi halamannya sekarang "Pipa Air" — tidak di-rename, sama seperti alasan `tambahkali` tetap dipertahankan namanya) — simulasi **aliran air lewat pipa & gerbang**, pengenalan intuitif AND/OR/NOT **tanpa istilah formal**. Awalnya dibuat dengan tema saklar+lampu dan bentuk tetap (fixed shapes) serta "saklar terbalik" untuk NOT, tapi diganti total setelah masukan: tema lampu/saklar dirasa membingungkan, dan "saklar terbalik" terasa aneh secara konsep.
  ...
  - **Satu akar, banyak daun (multi-ember)** — `LEVEL_PARAMS` (8 soal) adalah campuran dua bentuk config: 4 soal pertama config lama `{n, depth, invert}` = satu pohon, satu ember (`generateLevel` membungkusnya jadi `{shared: null, branches: [satu branch]}`); 4 soal terakhir config `{sharedN, branches: [...]}` = satu topologi gerbang **bersama** (opsional, `sharedN` gerbang) yang bercabang ke 2-3 **branch** independen, masing-masing dengan topologi & `goal` sendiri, berakhir di ember bernomor sendiri. Ini BUKAN graf/DAG — tetap pohon (satu akar, banyak daun), jadi proof `assignForTarget` tetap berlaku utuh per-cabang.
