...
- `sirkuit-sederhana/` (folder historis, tapi judul & isi halamannya sekarang "Pipa Air" — tidak di-rename, sama seperti alasan `tambahkali` tetap dipertahankan namanya) — simulasi **aliran air lewat pipa & gerbang**, pengenalan intuitif AND/OR/NOT **tanpa istilah formal**. Awalnya dibuat dengan tema saklar+lampu dan bentuk tetap (fixed shapes) serta "saklar terbalik" untuk NOT, tapi diganti total setelah masukan: tema lampu/saklar dirasa membingungkan, dan "saklar terbalik" terasa aneh secara konsep.
  - **Topologi acak generik** (bukan bentuk tetap) — `randomTopology(n, depth, invertBudget, nextId)` membangun pohon `{type:'switch'|'series'|'parallel', ...}` secara rekursif: tiap simpul internal punya 2 anak yang ukurannya (jumlah gerbang) dibagi acak, tipe (seri/paralel) dipilih acak, sehingga seri & paralel bisa bergabung berulang dalam kombinasi yang bervariasi. `LEVEL_PARAMS` menaikkan `n` (jumlah gerbang, 1→5), `depth` (kedalaman campuran, 0→3), dan `invert` (jumlah gerbang terbalik) selaras index soal 0-7. Kedua tujuan (isi ember / jangan isi ember) matematis selalu bisa dicapai untuk topologi apa pun (dibuktikan via induksi: series butuh semua anak "true"-achievable, parallel cukup satu) — makanya tidak perlu BFS solvability check seperti `labirin-kode`.
  - **Render generik via SVG**, bukan CSS flexbox — `measure(node)` menghitung lebar (unit seri) & tinggi (jumlah jalur paralel) tiap simpul secara rekursif, `renderNode()` menggambar ulang secara rekursif ke koordinat SVG (garis `<line>` sebagai pipa, gerbang sebagai `<button>` HTML di dalam `<foreignObject>` supaya tetap bisa di-tap seperti elemen HTML biasa). Pipa yang sedang mengalir (`evalNode()` true di segmen itu) diberi class `.live` (biru terang + glow); yang tidak, abu-abu pudar — termasuk pewarnaan per-cabang di simpul paralel (tiap cabang independen, bukan cuma satu warna untuk semua).
  ...
  - Anak men-tap gerbang untuk toggle (live update pipa tiap toggle, tanpa tombol "jalankan" terpisah), lalu tekan "✅ Cek" untuk submit ke `goal` (isi ember / jangan isi ember) soal itu. Tidak ada penalti untuk jawaban salah (mendorong bereksperimen), mengikuti filosofi `labirin-kode`.
  - **Kondisi awal gerbang sengaja dibuat berlawanan dari `goal`** lewat `assignForTarget(node, !goal, initState)` (bukan `Math.random()` polos per gerbang) — supaya soal tidak pernah kebetulan sudah benar saat dimuat (anak wajib mengubah minimal satu gerbang dulu sebelum bisa "Cek" dengan benar). `assignForTarget()` membangun mundur dari hasil yang diinginkan secara rekursif (series/parallel tahu persis kombinasi anak mana yang "menentukan" hasil, sisanya diacak bebas) — dijamin matematis lewat proof yang sama seperti soal selalu solvable, diverifikasi lewat simulasi 40.000 kombinasi tanpa satu pun re-match.
